...
当系统决定掷出 **A 组** (1-5) 点数时，不再使用均匀分布的随机数，而是采用 **“两次随机取最大值”** 的算法：
$$
\text{Roll} = \max(\text{Random}(1, 5), \text{Random}(1, 5))
$$
这使得掷出 4 和 5 的概率大幅增加，掷出 1 和 2 的概率降低，从而加快棋子在路上的移动速度。
...
*   **升温期 ($2 \le t < 4$)**: $C_t$ 从 0.3 线性增加至 1.0，公式为：$C_t = 0.3 + 0.7 \times \frac{t-2}{2}$
*   **激战期 ($4 \le t < 6$)**: $C_t = 1.0$ (全速推进，鼓励吃子)
*   **收官期 ($6 \le t < 9$)**: $C_t$ 从 1.0 线性降低至 0.2，公式为：$C_t = 1.0 - 0.8 \times \frac{t-6}{3}$
