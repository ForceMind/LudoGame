...
3.  **吃子干预 (Capture Intervention)**:
    *   当玩家（**包括 Bot**）有机会吃掉对手棋子时，系统会根据当前局势动态计算触发概率。
    *   **动态概率**: 胜率越低，触发概率越高（雪中送炭）；胜率越高，触发概率越低（锦上添花）。
    *   **连续衰减**: 如果在同一回合内连续触发吃子干预（例如吃子后获得奖励回合再次吃子），每次触发后概率降低 **10%**，防止过度滚雪球。

4.  **死局保底 (Failsafe Mechanism)**:
    *   **触发条件**: 当任意玩家（人类或 Bot）连续 **10-20** 轮（随机阈值）因掷不出合适点数而无法移动任何棋子时。
    *   **效果**: 系统强制生成一个有效点数。
    *   **优先级**: 
        1.  优先生成能直接到达终点的点数。
        2.  如果无法到达终点，生成任意一个能让棋子移动的点数。
    *   **目的**: 防止游戏因运气极差而陷入死局，确保游戏流畅进行。
...
Bot 会模拟每一个可移动棋子的走法，并计算得分。值得注意的是，高权重的行为（进终点、吃子）通常也伴随着**额外回合奖励**，这进一步强化了 Bot 采取这些行动的倾向。

1.  **进终点 (Finish) - 权重: 200**
    *   如果这一步能让棋子到达终点，给予最高优先级。（同时获得额外回合）

2.  **吃子 (Capture) - 权重: 100**
    *   如果这一步能吃掉对手的棋子，给予高优先级。这既能阻碍对手，又能增加自己的优势。（同时获得额外回合）
